    ...
    for (let c = 0; c < coluna; c++) {
        for (let r = 0; r < linha; r++) {
            if(blocos[c][r]. status == 1){
                ...
                const blocoY = r * (altura + padding) + top;
                blocos[c][r].x = blocoX;
                blocos[c][r].y = blocoY;
                ctx.beginPath;
...
function posicaoBola() {
    ...
    function mouseMecher(e){
        const relativo = e.clintX = canvas.offsetLeft;
    ...
    function desenho(){
        ctx.clerRect(0,0,canvas.width, canvas.height);
        desenhoBola();
        desenhoBlocos();
        desenhoRaquete();
        colisao();
